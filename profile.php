...
$check_user_data = $db->prepare("SELECT * FROM users WHERE user_id={$_GET['sef']}");
$check_user_data->execute(array());
$pull_user_data = $check_user_data->fetch(PDO::FETCH_ASSOC);

$check_blockedUser = $db->prepare("SELECT * FROM users_blocks WHERE blocking_user=:blocking_user AND blocked_user=:blocked_user");
$check_blockedUser->execute(array('blocking_user'=>$_SESSION['user_id'],'blocked_user'=>$_GET['sef']));
$blocked_user = $check_blockedUser->fetch(PDO::FETCH_ASSOC);
		...
		<div class="profile-sideoptions-list">
			<div class="profile-edit-list">
				...
				<ul>
					<li>
						<a href="#" 
						id="user-page-block-btn" 
						data-user="<?php echo $pull_user_data['user_id']; ?>"
						data-block="<?php echo $blocked_user['block_id'] > 0 ? "true" : "false" ?>"
						>
						<?php echo $blocked_user['block_id'] > 0 ? "Engeli kaldır" : "Engelle" ?></a>
					</li>
					<li><a href="#" id="user-page-complaint-btn" data-user="<?php echo $pull_user_data['user_id']; ?>">Şikayet et</a></li>
				</ul>
			</div>
		</div>
	<?php  } ?>
